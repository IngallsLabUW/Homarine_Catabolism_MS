# Purpose: Quality control on all targeted datasets for the G4 Metabolomics

# History:
# Date	     Remarks
# 20240524   K Heal changing to work with G4 data
...
outer_file_loc <- here("data", "raw", "metabolomics", "G4", "particulate", "mzml_skyline")
save_dir <- here("data", "intermediate", "metabolomics", "g4", "targeted")
...
QE_QC(
  dat_filename = here(
    outer_file_loc,
    "negative",
    "HILIC_QE_NEG_G4_D3_Homarine_Fate.csv"
  ),
  std_flag <- NA,
...
QE_QC(
  dat_filename = here(
    outer_file_loc,
    "positive",
    "HILIC_QE_POS_G4_D3_Homarine_Fate.csv"
  ),
  std_flag <- NA,
  blank_flag <- "_Blk_Blk_",
  sn_min = 3,
