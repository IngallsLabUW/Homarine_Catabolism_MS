...
growth.plot <- ggplot(hom.growth.hm.dat, aes(x = "Growth", y = reorder(isolate, -order), fill = hom_growth_status)) +
...
hom.gene.plot <- ggplot(gene.plot.dat.order.hom, aes(x = gene, y = reorder(isolate, -order), fill = as.factor(presence))) +
    geom_tile(color = "black", width = 1) +
...
mgd.gene.plot <- ggplot(gene.plot.dat.order.mgd, aes(x = gene, y = reorder(isolate, -order), fill = as.factor(presence))) +
    geom_tile(color = "black", width = 1) +
    scale_fill_manual(values = c("white", "#b0a5cc")) +
    theme_minimal() +
    ylab("Isolate")  +
...
sox.gene.plot <- ggplot(gene.plot.dat.order.sox, aes(x = gene, y = reorder(isolate, -order), fill = as.factor(presence))) +
    geom_tile(color = "black", width = 1) +
    scale_fill_manual(values = c("white", "#b0a5cc")) +
    theme_minimal() +
    ylab("Isolate")  +
...
genus.plot <- ggplot(gene.plot.dat.order, aes(x = "Genus", y = reorder(isolate, -order), fill = Family)) +
  geom_tile(color = "black", width = 1) +
  scale_fill_manual(values = c("#f8b767", "#fcdcb5", "#dd8fb9", "#f5d6e5", "#16afca", 
...
#combine plots
combined.plot <- (growth.plot | hom.gene.plot | 
                      mgd.gene.plot | 
                      sox.gene.plot | 
                      class.plot |  genus.plot) +
...
iso.supp.table <- gene.plot.dat.order %>%
    ...
    pivot_wider(id_cols = c(isolate, hom_growth_status, Kingdom, Phylum, Class, Order, Family, Genus, Species), 
