...
process_class_data <- function(class_data) {
  class_data %>%
    ...
    filter(hmm_profile %in% c("OBi1_02316_SPO3189")) %>%
    rowwise() %>%
...
create_catabolic_genes_with_chla_plot <- function(class_data, chla_data, station_data, title) {
  ...
  chla_scale_factor <- (max_transcripts / max_chla) * 2  # ← Shrink chl-a height by half
  ...
  ggplot() +
    ...
    scale_x_continuous(breaks = seq(24, 40, by = 4), limits = c(24, 42)) +
    scale_y_continuous(
      name = expression("homB transcripts (x10"^7~" per Liter)"),
      labels = function(x) x / 10^7,
      limits = c(0, 12e7),
      expand = c(0.000, 0),
...
# === Load Input Files ===
class_data_files <- c("C:/SynologyDrive/UW/UW_Anitra_Ingalls/Homarine_project/hmmr_profiles/HMMR_G5/Gradients_3/SPO3188_SPO3189_combined_G3_revised_annotations_with_metadata_final.csv")
