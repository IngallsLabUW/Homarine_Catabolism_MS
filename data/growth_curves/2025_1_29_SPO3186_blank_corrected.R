...
# Load OD600 data
OD600 <- fread('2023_8_25_Growth_of_SPO3186_on_Hom_GBT_DMSP_Carn_Nmetglut.csv', header=T)

# Convert Time to seconds and hours
OD600$TimeS <- period_to_seconds(lubridate::hms(OD600$Time))
OD600$TimeH <- OD600$TimeS / 3600

# Define blank wells identified from analysis
blank_wells <- c('A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'A8', 'A9', 'A10', 'A11', 'A12', 'G11', 'G12', 'H11', 'H12')

# Compute the average blank OD600 at each time point
OD600[, BlankMean := rowMeans(.SD, na.rm = TRUE), .SDcols = blank_wells]

# Identify sample wells (excluding Time, Temp, and blank wells)
sample_wells <- setdiff(names(OD600), c('Time', 'Temp', 'BlankMean', blank_wells))

# Subtract blank OD600 from all sample wells and ensure values are non-negative
OD600[, (sample_wells) := lapply(.SD, function(x) pmax(0, x - BlankMean)), .SDcols = sample_wells]
...
ggplot() +
  ...
  scale_y_continuous(expand = c(0, 0)) + theme(legend.position = "none")  + ylim(NA,.12)
...
ggplot() +
  ...
  theme(legend.position = "none")  + ylim(NA,.12)
